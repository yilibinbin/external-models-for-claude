...
- Unknown flags on the review commands and the `task` subcommand (used by `/codex:rescue`) are rejected. Use `--` before prompt or focus text that intentionally starts with flag-like tokens.
