...
const REVIEW_GATE_ENV = "CLAUDE_FOR_CLAUDE_REVIEW_GATE";
...
const UNTRACKED_BYTES_PER_FILE = 64 * 1024;
const UNTRACKED_TOTAL_EXCERPT_BYTES = 1024 * 1024;
let claudeHelpReportCache = null;
...
function claudeCommand() {
  const configuredPath = process.env[CLAUDE_CLI_PATH_ENV];
  ...
  for (const candidate of candidateClaudeCommands()) {
    if (isExecutable(candidate)) {
      return candidate;
    }
  }
...
function runClaude(args, options = {}) {
  const invocation = normalizePosixScriptInvocation(claudeCommand(), args);
...
function claudeHelpReport() {
  const result = runClaude(["--help"]);
  const help = result.stdout || result.stderr || "";
  const requiredFlags = ["--print", "--safe-mode", "--no-session-persistence", "--tools", "--input-format", "--output-format"];
  ...
  return {
    ...
    missing,
    requiredFlags,
    capabilities: claudeCapabilitiesFromHelp(help),
    error: result.status === 0 ? "" : (result.stderr || result.error || "claude --help failed").trim()
  };
}

function claudeCapabilitiesFromHelp(help) {
...
function currentClaudeCapabilities() {
  if (!claudeHelpReportCache) {
    claudeHelpReportCache = claudeHelpReport();
  }
  return claudeHelpReportCache.capabilities;
}

function requireClaudeCapability(capability, flag) {
  const capabilities = currentClaudeCapabilities();
  if (!capabilities[capability]) {
    throw new Error(`Claude CLI does not report support for ${flag}. Run setup to inspect available capabilities.`);
  }
}
...
function applyDefaultClaudeModel(args, env = process.env) {
  if (args.model !== undefined) {
    args.model = normalizeClaudeModel(args.model);
    if (!args.model) {
      throw new Error("Missing value for --model.");
    }
    return args;
  }
  const model = normalizeClaudeModel(env.CLAUDE_FOR_CLAUDE_MODEL);
  if (model) {
    args.model = model;
  }
  return args;
...
function printUsage(commandName = "") {
  ...
  process.stdout.write(`Usage: claude-companion.mjs${command} [args]\n`);
  if (!commandName) {
    process.stdout.write(`Commands: ${publicCommands().join(", ")}\n`);
    process.stdout.write("Run `claude-companion.mjs <command> --help` for command-specific usage.\n");
    return;
  }
  switch (commandName) {
    case "review":
      process.stdout.write("Review current git context with Claude in read-only mode.\n");
      process.stdout.write("Options: [--scope auto|working-tree|branch] [--base <ref>] [--path <path>] [--json|--structured] [focus]\n");
      break;
    case "adversarial-review":
      process.stdout.write("Run an adversarial read-only Claude review.\n");
      process.stdout.write("Options: [--scope auto|working-tree|branch] [--base <ref>] [--path <path>] [--json] [focus]\n");
      break;
    case "multi-review":
      process.stdout.write("Run Claude role-based read-only review without editing files.\n");
      ...
      break;
    case "plan-review":
      process.stdout.write("Run Claude role-based read-only review of a saved plan file.\n");
      process.stdout.write("Options: --plan <file> [--roles <list>|--role-pack <pack>] [--scorecard [--json]] [focus]\n");
      break;
    case "assisted-review":
      process.stdout.write("Run a bounded advisory Claude scorecard review loop.\n");
    ...
    case "real-smoke":
      ...
      process.stdout.write("Options: [--quick|--full] [--model <name>] [--roles <list>] [--timeout-seconds <n>] [--include-multi] [--include-native]\n");
      break;
    default:
      process.stdout.write("Use this command with its documented options. This help path does not invoke Claude.\n");
      break;
  }
}
...
function textExcerptForFile(root, relativePath, maxBytes = UNTRACKED_BYTES_PER_FILE) {
  if (!isSafeRelativePath(relativePath)) {
    return `Untracked file: ${relativePath}\n[skipped unsafe path]`;
  }
  const readLimit = Math.max(0, Math.min(UNTRACKED_BYTES_PER_FILE, Number(maxBytes) || 0));
  if (readLimit <= 0) {
    return `Untracked file: ${relativePath}\n[skipped excerpt budget exhausted]`;
  }
  ...
  try {
    ...
    buffer = Buffer.alloc(Math.min(fdStat.size, readLimit + 1));
    ...
    try {
      ...
      // Best-effort cleanup after a bounded read.
    }
  }
  if (buffer.includes(0)) {
    return `Untracked file: ${relativePath}\n[skipped binary file]`;
  }
  const excerpt = buffer.subarray(0, readLimit).toString("utf8");
  const truncated = fdStat.size > readLimit ? "\n[untracked file excerpt truncated]" : "";
  return `Untracked file: ${relativePath}\n${excerpt}${truncated}`;
}

function untrackedContext(root, pathArgs = []) {
  const listing = run("git", ["ls-files", "--others", "--exclude-standard", ...pathArgs], {
    cwd: root,
    timeout: REVIEW_GIT_TIMEOUT_MS,
    killSignal: "SIGKILL"
  });
  if (listing.status !== 0) {
    return formatCommandResult("git ls-files --others --exclude-standard", listing);
  }
  ...
  const excerpts = [];
  let remainingBytes = UNTRACKED_TOTAL_EXCERPT_BYTES;
  for (const file of files) {
    if (excerpts.length >= UNTRACKED_FILE_LIMIT || remainingBytes <= 0) {
      break;
    }
    const overhead = Buffer.byteLength(`Untracked file: ${file}\n\n[untracked file excerpt truncated]`, "utf8") + 2;
    const contentBudget = remainingBytes - overhead;
    if (contentBudget <= 0) {
      break;
    }
    const excerpt = textExcerptForFile(root, file, contentBudget);
    excerpts.push(excerpt);
    remainingBytes -= Buffer.byteLength(excerpt, "utf8") + 2;
  }
  if (files.length > excerpts.length) {
    excerpts.push(`[untracked file excerpts truncated after ${excerpts.length} of ${files.length} files; all untracked paths are listed above]`);
  }
  return [...pathList, "", ...excerpts].join("\n\n");
}
  ...
  const branchNameOnly = includeBaseBranch
    ...
    : includeHeadNameOnly && headExists
      ...
      : includeHeadNameOnly && status
        ? changedFilesFromStatus(status)
        : null;
  const rootResult = includeWorkingTree ? git(["rev-parse", "--show-toplevel"]) : null;
  const root = rootResult?.status === 0 ? rootResult.stdout.trim() : process.cwd();
  const untracked = includeWorkingTree ? untrackedContext(root, pathArgs) : "";
...
function claudePrintArgs(prompt, options = {}) {
  const args = [
    "-p",
    "--safe-mode",
...
function claudePrint(prompt, options = {}) {
  ...
  }, () => claudePrintUnguarded(prompt, options));
}

function claudePrintUnguarded(prompt, options = {}) {
  const result = runClaude(claudePrintArgs(prompt, options), { timeout: options.timeout, cwd: options.cwd, input: prompt });
  if (result.status !== 0) {
    return result;
  }
...
function reportRoot(env = process.env) {
  const base = env.CLAUDE_PLUGIN_DATA
    ...
    : env.CLAUDE_FOR_CLAUDE_DATA
    ? path.resolve(env.CLAUDE_FOR_CLAUDE_DATA)
    : path.join(os.homedir(), ".claude", "claude-for-claude");
...
function sanitizeReportPayload(payload) {
  ...
  const safe = {
    ...
    claudeAvailable: Boolean(payload.claudeAvailable),
...
function printCapabilities() {
  const preflight = claudeHelpReport();
  const cwd = process.cwd();
...
function realSmokeOptions(rawArgs = []) {
  ...
  for (let index = 0; index < tokens.length; index += 1) {
    ...
    if (token === "--quick") {
      ...
      options.mode = "full";
      options.includeMulti = true;
      options.includeNative = true;
    } else if (token === "--include-multi") {
      options.includeMulti = true;
  ...
  if (!options.timeoutMs) {
    options.timeoutMs = options.mode === "full" || options.includeMulti || options.includeNative ? 10 * 60 * 1000 : 5 * 60 * 1000;
  }
  if (!options.roles) {
    options.roles = options.mode === "quick" ? ["tests"] : ["correctness", "tests"];
  }
  if (!options.modelFromArg) {
    options.model = normalizeRealSmokeModel(process.env.CLAUDE_FOR_CLAUDE_REAL_SMOKE_MODEL || process.env.CLAUDE_FOR_CLAUDE_MODEL || "");
  }
...
function createRealSmokeWorkspace() {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "claude-for-claude-smoke-"));
  ...
  try {
    ...
    const commit = spawnSync("git", ["-c", "user.name=Claude for Claude Smoke", "-c", "user.email=claude-for-claude-smoke@example.invalid", "-c", "commit.gpgsign=false", "commit", "-m", "Initial smoke fixture"], gitOptions);
    if (commit.status !== 0) {
...
async function runRealSmoke(rawArgs = []) {
  if (process.env.CLAUDE_FOR_CLAUDE_REAL_SMOKE !== "1") {
    console.error("real-smoke is opt-in. Set CLAUDE_FOR_CLAUDE_REAL_SMOKE=1 to run live Claude CLI smoke checks.");
    process.exit(2);
  }
  let options;
  try {
  ...
  const payload = {
    ...
    includeMulti: options.includeMulti,
    includeNative: options.includeNative,
    model: options.model || "claude-cli-default",
...
function claudeHostForbiddenStrings() {
  return [
    ...
    ["CLAUDE", ["FOR", "COD", "EX"].join("_")].join("_"),
    [".", "codex", "-plugin"].join(""),
    [".", "codex", "/"].join(""),
    ["claude", "for", "codex"].join("-"),
    ["Claude", "for", "Codex"].join(" ")
  ];
}
...
function checkClaudeHooksSafety() {
  ...
  const gateWrapper = fs.readFileSync(path.join(ROOT_DIR, "hooks", "claude-review-gate.mjs"), "utf8");
...
function checkQualityLoopSurface() {
  const failures = [];
  const companion = fs.readFileSync(path.join(ROOT_DIR, "scripts", "claude-companion.mjs"), "utf8");
  const runtime = fs.readFileSync(path.join(ROOT_DIR, "scripts", "lib", "claude-runtime.mjs"), "utf8");
  ...
  const expectedFiles = [
    ...
    "skills/claude-assisted-review/SKILL.md"
  ];
  ...
  if (!runtime.includes("normalizePosixScriptInvocation")) {
    failures.push("claude runtime missing POSIX script invocation normalization");
  }
...
function checkResourceGovernorSafety() {
  ...
  const requiredGovernorTokens = [
    "CLAUDE_FOR_CLAUDE_RESOURCE_LOCK_DIR",
    "CLAUDE_FOR_CLAUDE_GLOBAL_MAX_MODEL_CALLS",
    "CLAUDE_FOR_CLAUDE_GLOBAL_MAX_BACKGROUND_JOBS",
    "CLAUDE_FOR_CLAUDE_MULTI_REVIEW_MAX_PARALLEL",
  ...
  const requiredCompanionTokens = [
    ...
    "multiReviewConcurrency()",
    "sequential Claude CLI role review",
    "capacityBlockedMessage(\"claude-for-claude\""
  ];
...
function checkGithubActionsCi() {
  ...
  try {
    ...
    if (!plain.includes("git -C \"$marketplace_dir\" fetch --depth 1 origin \"$CLAUDE_FOR_CLAUDE_RELEASE_REF\"")) failures.push("github actions missing immutable Claude release ref checkout");
  ...
  for (const file of scriptTextFiles()) {
    ...
    for (const token of dangerous) {
      if (text.includes(token)) {
        failures.push(`${file} contains forbidden Claude review flag ${token}`);
      }
    }
  }
  return { ok: failures.length === 0, failures };
...
function scriptTextFiles() {
  const files = [path.join(ROOT_DIR, "scripts", "claude-companion.mjs")];
...
function checkNativeOrchestrationSafety() {
  ...
  const companion = fs.readFileSync(path.join(ROOT_DIR, "scripts", "claude-companion.mjs"), "utf8");
  const readme = fs.readFileSync(path.join(ROOT_DIR, "README.md"), "utf8");
  const multiSkill = fs.readFileSync(path.join(ROOT_DIR, "skills", "claude-multi-review", "SKILL.md"), "utf8");
  const rolePackSkill = fs.readFileSync(path.join(ROOT_DIR, "skills", "claude-role-packs", "SKILL.md"), "utf8");
  const hooks = fs.readFileSync(path.join(ROOT_DIR, "hooks", "hooks.json"), "utf8");
  const workflow = renderWorkflow(ROOT_DIR);
  ...
  if (!multiSkill.includes("--agent-team native-agents")) failures.push("claude-multi-review skill missing --agent-team native-agents");
  if (!rolePackSkill.includes("--agent-team native-agents")) failures.push("claude-role-packs skill missing --agent-team native-agents");
...
function checkCoordinationFixtures() {
  const failures = [];
  const temp = fs.mkdtempSync(path.join(os.tmpdir(), "claude-for-claude-coordination-"));
  ...
  try {
    ...
    const message = postMailboxMessage(workspace, {
      ...
      source: "manual",
      summary
    }, { ...process.env, CLAUDE_FOR_CLAUDE_DATA: data });
    ...
    const first = claimLease(workspace, { path: "file.txt", role: "correctness", ttl: "60s", mode: "manual" }, { ...process.env, CLAUDE_FOR_CLAUDE_DATA: data });
    const second = claimLease(workspace, { path: "file.txt", role: "security", ttl: "60s", mode: "manual" }, { ...process.env, CLAUDE_FOR_CLAUDE_DATA: data });
...
function runConcurrentLeaseFixture(workspace, data) {
  const script = `
    ...
    const childCode = \`
      ...
      const result = m.claimLease(workspace, {path:"concurrent.txt", role, ttl:"60s", mode:"manual"}, {CLAUDE_FOR_CLAUDE_DATA: data, HOME: process.env.HOME});
...
function releaseCheckProviderFixtures() {
  const temp = fs.mkdtempSync(path.join(os.tmpdir(), "claude-for-claude-release-check-"));
  const workspace = path.join(temp, "workspace");
  ...
  try {
    fs.writeFileSync(config, JSON.stringify({
      providers: {
        safe: {
          command: [provider],
          env: { CLAUDE_CONTEXT_PROVIDER_MODE: "summary" }
        }
      },
    ...
    const resolved = {
      env: { ...process.env, CLAUDE_FOR_CLAUDE_CONTEXT_CONFIG: config }
    };
...
async function claudePrintAsync(prompt, options = {}) {
  return await withResourceLeaseAsync("model-call", {
    command: "claude",
    ttlMs: (options.timeout || 15 * 60 * 1000) + 60_000
  }, async () => claudePrintAsyncUnguarded(prompt, options));
}

async function claudePrintAsyncUnguarded(prompt, options = {}) {
  const result = await runClaudeAsync(claudePrintArgs(prompt, options), { timeout: options.timeout, cwd: options.cwd, input: prompt });
  if (result.status !== 0) {
    return result;
  }
  const parsed = parseClaudeJson(result.stdout);
...
function parseClaudeJson(stdout) {
  ...
  try {
    ...
    return { ok: false, response: "", error: `Invalid Claude JSON output: ${error.message}` };
  }
  if (parsed.error) {
  ...
  const response = typeof parsed.response === "string"
    ...
    : typeof parsed.result === "string"
      ...
      : "";
  if (!response) {
    return { ok: false, response: "", error: "Claude JSON output did not include a string result." };
  }
...
function startBackgroundJob(command, rawArgs) {
  ...
  if (!resourceLease.ok) {
    console.error(capacityBlockedMessage("claude-for-claude", resourceLease));
    process.exit(75);
  }
...
function handleReserveJob(rawArgs) {
  ...
  const resourceLease = acquireResourceLease("background-job", { command, transferable: true });
  if (!resourceLease.ok) {
    throw new Error(capacityBlockedMessage("claude-for-claude", resourceLease));
  }
  const workerCommand = [
  ...
  const roleResults = results.map(({ role, result }) => {
    if (result.status !== 0) {
      return {
        role: role.name,
        status: "error",
        error: (result.stderr || result.error || "Claude role review failed").trim()
      };
    }
    try {
      return {
...
function nativeAgentName(roleName) {
  return `cfc_${roleName.replace(/[^a-zA-Z0-9_]/g, "_")}`;
}
...
function nativeAgentMarkdown(role) {
  return [
    ...
    `description: Claude for Claude ${role.name} read-only review subagent.`,
    "---",
    "",
    `You are the ${role.name} reviewer for Claude for Claude.`,
    "",
...
function writeNativeAgentWorkspace(roles) {
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "claude-for-claude-agents-"));
...
function nativeStructuredContract() {
  return [
    ...
    "Schema:",
    "{",
    '  "role_results": [{"agent":"cfc_role","role":"role name","status":"ok|error","text":"summary","error":""}],',
...
function validateNativeStructuredAggregate(value, expectedRoles) {
  ...
  for (const item of value.role_results) {
    ...
    if (typeof item.agent !== "string" || !item.agent.startsWith("cfc_")) {
...
function extractNativeStructuredObject(rawOutput) {
  ...
  if (!text) {
    throw new Error("Claude returned empty native structured output.");
  }
  try {
  ...
  throw new Error("Claude output did not contain a native structured JSON object.");
}
...
function rescuePrompt(args) {
  ...
  return [
    "<task>Diagnose a stuck or failed Claude Code implementation from Claude's independent read-only perspective.</task>",
    gitContext,
    "<rules>",
    "- Do not edit files.",
...
function hookTrustKeyMatches(key) {
  return key.includes("claude-for-claude")
...
function buildSetupReport(actionsTaken = []) {
  ...
  return {
    ...
    claudeCapabilities: preflight.capabilities,
    capabilities: {
      claude: preflight.capabilities,
      requiredFlags: {
        ok: preflight.ok,
...
function printSetup(rawArgs) {
  ...
  process.exit(report.claudeAvailable && report.claudePreflight.ok && report.gitAvailable && report.reviewGate.stateReadable ? 0 : 1);
}
...
function runRolesCommand(rawArgs) {
  ...
  try {
    if (subcommand === "list") {
      if (filtered.length) {
        throw new Error("Usage: claude-companion.mjs roles list [--json]");
      }
      const packs = listRolePacks();
      if (jsonOutput) {
  ...
  try {
    if (subcommand === "list") {
      if (rest.some((token) => token !== "--json")) {
        throw new Error("Usage: claude-companion.mjs mailbox list [--json]");
      }
    ...
    if (subcommand === "post") {
      ...
      if (!options.jobId || !options.summary) {
        throw new Error("Usage: claude-companion.mjs mailbox post --job-id <id> --summary <text> [--role <role>] [--json]");
      }
...
function runLeasesCommand(rawArgs) {
  ...
  try {
    if (subcommand === "list") {
      if (rest.some((token) => token !== "--json")) {
        throw new Error("Usage: claude-companion.mjs leases list [--json]");
      }
    ...
    if (subcommand === "claim") {
      ...
      if (!options.path) {
        throw new Error("Usage: claude-companion.mjs leases claim --path <path> --role <role> --ttl <duration> [--job-id <id>] [--json]");
      }
...
function validateClaudeSessionOptions(args) {
  if (args.resume && args.fresh) {
    throw new Error("Choose either --resume or --fresh, not both.");
  }
  if (args.resume) {
    requireClaudeCapability("resume", "--resume");
  }
  if (args.sessionId) {
    requireClaudeCapability("sessionId", "--session-id");
  }
  if (args.worktree) {
    requireClaudeCapability("worktree", "--worktree");
  }
}

function warnGate(message) {
  process.stderr.write(`[claude-for-claude review-gate] ${message}\n`);
}
...
async function runReviewGate(rawArgs) {
  ...
  if (args.contextProvider !== undefined || args.contextBudget !== undefined || args.contextTimeoutMs !== undefined || args.contextStrict) {
    ...
    if (args.contextStrict && context.metadata.contextStatus !== "available" && context.metadata.contextStatus !== "disabled") {
      warnGate(`context provider unavailable in strict mode; allowing stop without Claude: ${context.metadata.contextFailureReason}`);
      writeOperationReport({ command: "review-gate", cwd, status: 2, claudeAvailable: hasClaude(), contextMetadata: context.metadata, rolePack: args.rolePackSummary });
      return;
    }
    if (context.metadata.contextDegraded) {
      warnGate(`context degraded (${context.metadata.contextFailureReason}); gate decision will fail open unless Claude returns BLOCK`);
    }
  }
  const blocks = [];
  for (const role of roles) {
    ...
    if (result.status !== 0) {
      const detail = (result.stderr || result.error || result.stdout || "claude review failed").trim();
  ...
  writeOperationReport({ command: "review-gate", cwd, status: 0, claudeAvailable: hasClaude(), contextMetadata: context.metadata, rolePack: args.rolePackSummary });
}
...
async function runClaudeTask(kind, rawArgs) {
  ...
  try {
    ...
    if (args.write) {
      throw new Error("Claude for Claude is read-only; --write is not supported.");
    }
    if (args.effort) {
      throw new Error("--effort is not supported by Claude for Claude.");
    }
    ...
    validateClaudeSessionOptions(args);
  ...
  if (kind === "review" || kind === "adversarial-review") {
    ...
    if (args.contextStrict && context.metadata.contextStatus !== "available" && context.metadata.contextStatus !== "disabled") {
      ...
      writeOperationReport({ command: kind, cwd: process.cwd(), status: 2, claudeAvailable: hasClaude(), contextMetadata: context.metadata });
      process.exit(2);
    }
  }
  const prompt = kind === "plan" ? planPrompt(args) : kind === "rescue" ? rescuePrompt(args) : reviewPrompt(kind, args, context.block);
  const result = claudePrint(prompt, args);
  writeOperationReport({ command: kind, cwd: process.cwd(), status: result.status, claudeAvailable: hasClaude(), contextMetadata: context.metadata });

  if (result.status !== 0) {
    process.stderr.write(result.stderr || result.error || "claude review failed\n");
...
async function runClaudeMultiReview(rawArgs) {
  // Do not acquire an outer model-call lease here. Each role obtains its own
  // bounded lease in claudePrint/claudePrintAsync, and multiReviewConcurrency()
  // clamps role fan-out to the active model-call capacity.
  ...
  try {
    ...
    if (args.nativeAgents && args.rolePackSummary) {
      if (!rolePackNativeAgentsCompatible(args.rolePack)) {
        throw new Error(`Role pack "${args.rolePack.name}" is not compatible with Claude native agents.`);
      }
      ...
      if (!currentClaudeCapabilities().includeDirectories) {
        throw new Error("Claude native role-pack review requires Claude CLI --include-directories support.");
      }
    }
  } catch (error) {
    console.error(error.message || String(error));
  ...
  if (args.contextStrict && context.metadata.contextStatus !== "available" && context.metadata.contextStatus !== "disabled") {
    console.error(`Context provider unavailable in strict mode: ${context.metadata.contextFailureReason}`);
    writeOperationReport({ command: "multi-review", cwd: process.cwd(), status: 2, claudeAvailable: hasClaude(), contextMetadata: context.metadata, rolePack: args.rolePackSummary });
    process.exit(2);
  }
  let results;
  let nativeWorkspace = "";
  ...
  if (args.advisoryLeases) {
    if (!args.paths.length) {
      process.stderr.write("[claude-for-claude leases] --advisory-leases supplied without --path; skipping leases.\n");
    } else {
      for (const leasePath of args.paths) {
        try {
          leaseResults.push(claimLease(process.cwd(), {
            path: leasePath,
            role: args.nativeAgents ? "native-claude-subagents" : args.reviewRoles[0]?.name || "multi-review",
            ttl: "600s",
            mode: args.nativeAgents ? "native-agents" : "plugin-managed"
          }));
        } catch (error) {
  ...
  function writeMailbox(message) {
    ...
    try {
      ...
      process.stderr.write(`[claude-for-claude mailbox] ${error.message || String(error)}\n`);
    }
  }
      ...
      if (leaseResult.status === "claimed" && leaseResult.lease?.id) {
        try {
          ...
          if (released.status !== "released") {
            process.stderr.write(`[claude-for-claude leases] failed to release ${leaseResult.lease.id}: ${released.reason || released.status}\n`);
          }
        } catch (error) {
          process.stderr.write(`[claude-for-claude leases] failed to release ${leaseResult.lease.id}: ${error.message || String(error)}\n`);
        }
      }
    }
  }
  ...
  try {
    if (args.nativeAgents) {
      try {
        writeMailbox({
          role: "native-claude-subagents",
          mode: "native-agents",
          status: "started",
          summary: `Claude native-agent review started for roles: ${args.reviewRoles.map((role) => role.name).join(", ")}.`
        });
        nativeWorkspace = writeNativeAgentWorkspace(args.reviewRoles);
        emitProgress(args, "native-agents started");
        const result = claudePrint(nativeMultiAgentPrompt(args, gitContext, context.block), {
          ...args,
          cwd: nativeWorkspace,
          includeDirectories: currentClaudeCapabilities().includeDirectories ? [process.cwd()] : []
        });
        emitProgress(args, "native-agents finished", { status: result.status });
        results = [{ role: { name: "native-claude-subagents" }, result }];
        writeMailbox({
          role: "native-claude-subagents",
          mode: "native-agents",
          status: result.status === 0 ? "succeeded" : "failed",
          summary: `Claude native-agent review ${result.status === 0 ? "succeeded" : "failed"} with exit status ${result.status}.`
        });
        if (args.nativeStructured) {
          ...
          if (result.status !== 0) {
            ...
            nativeStructuredExit = {
              status: result.status || 1,
              stdout: "",
              stderr: result.stderr || result.error || "Claude native structured review failed\n",
              report: {
                command: "multi-review",
                cwd: process.cwd(),
                status: result.status || 1,
          ...
          if (!nativeStructuredExit) {
            try {
              ...
              nativeStructuredExit = {
                status: 1,
                stdout: "",
                stderr: `Invalid Claude native structured output: ${error.message || String(error)}\n`,
                report: {
                  command: "multi-review",
                  cwd: process.cwd(),
                  status: 1,
      ...
      args.orchestrationMode = concurrency <= 1 ? "sequential Claude CLI role review" : `parallel Claude CLI role fan-out (bounded ${concurrency} max)`;
    }
  } finally {
  ...
  const leaseMetadata = leaseSummary(leaseResults);
  const sections = [
    args.nativeAgents ? "# Claude Native Subagent Review" : "# Claude Multi-Agent Review",
    ...
    `orchestration: ${args.nativeAgents ? "Claude native subagents" : (args.orchestrationMode || "bounded Claude CLI role fan-out")}`,
...
async function runClaudePlanReview(rawArgs) {
  ...
  const sections = [
    ...
    "## Orchestration Summary",
    `roles requested: ${args.reviewRoles.map((role) => role.name).join(", ")}`,
    `orchestration: ${concurrency <= 1 ? "sequential Claude CLI plan review" : `parallel Claude CLI plan review (bounded ${concurrency} max)`}`,
    "exit policy: exits non-zero if any role fails; completed role output remains visible."
  ];
  process.stdout.write(`${sections.join("\n\n")}\n`);
  writeOperationReport({
...
async function runClaudeAssistedReview(rawArgs) {
  ...
  if (args.contextStrict && context.metadata.contextStatus !== "available" && context.metadata.contextStatus !== "disabled") {
    console.error(`Context provider unavailable in strict mode: ${context.metadata.contextFailureReason}`);
    writeOperationReport({ command: "assisted-review", cwd: process.cwd(), status: 2, claudeAvailable: hasClaude(), contextMetadata: context.metadata });
    process.exit(2);
  }
  ...
  const payload = {
    ...
    nextSuggestedCommand: "claude-review --scorecard --json"
  };
  process.stdout.write(`${JSON.stringify(payload, null, 2)}\n`);
  writeOperationReport({
...
function printSessions() {
  ...
  if (!capabilities.listSessions) {
    process.stdout.write(`${JSON.stringify({
      available: false,
      reason: "Claude CLI does not report --list-sessions support."
    }, null, 2)}\n`);
    process.exit(1);
  }
  const result = runClaude(["--list-sessions"]);
  process.stdout.write(`${JSON.stringify({
...
function runJobWorker(rawArgs) {
  ...
  if (!resourceLease.ok) {
    const message = capacityBlockedMessage("claude-for-claude", resourceLease);
    finishJob(process.cwd(), jobId, {
      status: 75,
      stdout: "",
      stderr: `${message}\n`,
  ...
  try {
    const result = spawnSync(process.execPath, [process.argv[1], job.command, ...(job.args ?? [])], {
      ...
      env: {
        ...process.env,
        CLAUDE_FOR_CLAUDE_JOB_WORKER: "1"
      },
      encoding: "utf8",
      maxBuffer: 20 * 1024 * 1024,
      timeout: 30 * 60 * 1000
...
export async function main(argv = process.argv.slice(2)) {
  ...
  switch (command) {
    ...
    case "plan":
      ...
      break;
    case "assisted-review":
      await runClaudeAssistedReview(rawArgs);
      break;
    case "rescue":
