...
const WORKFLOW_RELATIVE_PATH = path.join(".github", "workflows", "claude-for-claude-review.yml");
const DEFAULT_RELEASE_REF = "claude-for-claude--v0.1.0";
const DEFAULT_TIMEOUT_MINUTES = 30;
const LOCAL_PATH_PATTERN = /\/Users\/[A-Za-z0-9._/-]+|\/private\/var\/folders\/[A-Za-z0-9._/-]+|[A-Za-z]:\\Users\\[A-Za-z0-9._\\/-]+/;
const MARKER = "<!-- claude-for-claude-review -->";
...
function readTemplate(pluginRoot) {
  return fs.readFileSync(path.join(pluginRoot, "templates", "github-actions", "claude-for-claude-review.yml"), "utf8");
}
...
export function renderWorkflow(pluginRoot, options = {}) {
  ...
  const modelArg = model ? `--model ${shellSingleQuote(model)}` : "";
  ...
  const annotationStep = annotations ? [
    ...
    "        run: |",
    "          set -euo pipefail",
    "          node \"$CLAUDE_PLUGIN_ROOT/scripts/claude-companion.mjs\" github-actions render-annotations --input claude-for-claude-review.json > claude-for-claude-annotations.json",
    "",
    ...
    "          script: |",
    "            const fs = require('fs');",
    "            const annotations = JSON.parse(fs.readFileSync('claude-for-claude-annotations.json', 'utf8'));",
...
export function validateWorkflow(text, options = {}) {
  ...
  const checks = [
    ...
    result(body.includes("npm install -g @anthropic-ai/claude-code"), "claude-cli-install"),
    result(body.includes("https://github.com/yilibinbin/external-models-for-claude") && body.includes("claude plugin marketplace add \"$marketplace_dir\" --scope user"), "marketplace-install"),
    result(body.includes("claude plugin install claude-for-claude@external-models-for-claude --scope user"), "plugin-install"),
    result(body.includes("CLAUDE_FOR_CLAUDE_RELEASE_REF:") && body.includes("git -C \"$marketplace_dir\" fetch --depth 1 origin \"$CLAUDE_FOR_CLAUDE_RELEASE_REF\""), "immutable-release-ref"),
    result(body.includes("claude plugin list --json") && body.includes("installPath") && body.includes("claude-for-claude@external-models-for-claude") && body.includes("$CLAUDE_PLUGIN_ROOT/scripts/claude-companion.mjs"), "plugin-root-resolved"),
    result(!body.includes("node plugins/claude-for-claude/scripts/"), "no-repo-relative-runtime-path"),
...
export function reviewToAnnotations(review) {
  ...
  for (const finding of findings) {
    ...
    annotations.push({
      ...
      title: sanitizeText(finding.title ?? "Claude for Claude finding"),
