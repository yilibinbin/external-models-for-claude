...
export function contextConfigPaths(env = process.env) {
  const paths = [];
  if (env.CLAUDE_FOR_CLAUDE_CONTEXT_CONFIG) {
    paths.push(path.resolve(env.CLAUDE_FOR_CLAUDE_CONTEXT_CONFIG));
  }
  if (env.CLAUDE_PLUGIN_DATA) {
    paths.push(path.join(path.resolve(env.CLAUDE_PLUGIN_DATA), "context", "providers.json"));
  }
  if (env.CLAUDE_FOR_CLAUDE_DATA) {
    paths.push(path.join(path.resolve(env.CLAUDE_FOR_CLAUDE_DATA), "context", "providers.json"));
  }
  paths.push(path.join(os.homedir(), ".claude", "claude-for-claude", "context", "providers.json"));
  return paths;
}
...
function validateProviderConfig(providerName, provider, configPath, workspaceRoot) {
  ...
  for (const [key, value] of Object.entries(env)) {
    if (!/^CLAUDE_CONTEXT_PROVIDER_[A-Z0-9_]+$/.test(key) || typeof value !== "string") {
...
function renderContext(providerName, parsed, workspaceRoot, budget) {
  ...
  const lines = [`<claude_context provider="${xmlEscape(providerName)}" status="available">`];
  let degraded = false;
  for (const item of items) {
    ...
    const candidate = `${lines.concat(block, ["</claude_context>"]).join("\n")}\n`;
...
export function unavailableContextBlock(providerName, reason) {
  return [
    `<claude_context provider="${xmlEscape(providerName)}" status="unavailable" reason="${xmlEscape(reason)}">`,
    "Requested Claude context was unavailable. Treat any context-dependent conclusion as degraded.",
    "</claude_context>"
  ].join("\n");
}
