<task>Run a native Claude subagent multi-review.</task>
<subagents>
{{SUBAGENTS}}
</subagents>
{{GIT_CONTEXT}}
{{CLAUDE_CONTEXT}}
<rules>
- Dispatch these Claude subagents in parallel when possible: {{AGENT_CALLS}}.
...
- Treat `<claude_context>` as advisory support only; never cite provider context as the sole evidence for a finding.
