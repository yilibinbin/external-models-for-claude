...
function notifyUnreadResults(cwd, sessionId) {
  ...
  process.stderr.write(`[claude-for-claude] Unread Claude job result: ${summary}. Run claude-result <job-id>.\n`);
}
...
try {
  ...
  process.stderr.write(`[claude-for-claude unread-result] ${error.message || String(error)}\n`);
}
