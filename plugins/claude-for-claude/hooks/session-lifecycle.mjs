...
function handleSessionEnd(input) {
  ...
  if (!sessionId) {
    process.stderr.write("[claude-for-claude lifecycle] SessionEnd missing session id; leaving jobs untouched.\n");
    return;
  }
...
try {
  ...
  process.stderr.write(`[claude-for-claude lifecycle] ${error.message || String(error)}\n`);
}
