...
description: Run a bounded Claude scorecard review loop that helps Claude Code decide whether current changes need more work.
---

# Claude Assisted Review

Use this skill after implementation when Claude Code needs a bounded Claude quality loop over the current git changes.
...
```bash
node "${CLAUDE_PLUGIN_ROOT}/scripts/claude-companion.mjs" assisted-review --taskset <taskset-id>
```
...
- Treat `needs-attention` as a signal for Claude Code to inspect and decide next fixes, not as an automatic blocker.
- If Claude fails, times out, or returns invalid scorecard JSON, report the failure explicitly.
