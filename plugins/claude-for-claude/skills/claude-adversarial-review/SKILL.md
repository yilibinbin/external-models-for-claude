...
description: Use Claude CLI to challenge Claude Code's implementation approach, assumptions, tradeoffs, and failure modes.
---

# Claude Adversarial Review

Use this skill for high-risk changes, architecture decisions, reliability-sensitive code, security-sensitive code, migrations, rollback-sensitive changes, or when Claude Code may be overconfident.
...
```bash
node "${CLAUDE_PLUGIN_ROOT}/scripts/claude-companion.mjs" reserve-job adversarial-review
```
...
- Use `--json` when Claude Code needs a machine-checked adversarial verdict object.
- Claude Code must accept or reject Claude findings with its own judgment before making follow-up edits.
