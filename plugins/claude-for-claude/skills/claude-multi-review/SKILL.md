...
description: Opt in to plugin-managed role fan-out Claude review from Claude Code for high-risk changes that need multiple read-only perspectives.
---

# Claude Multi Review

Use this skill when Claude Code needs several independent Claude review passes with different role directives. This is an opt-in fan-out review workflow, not automatic repair.

Default foreground execution runs plugin-managed parallel Claude CLI role fan-out. Add `--agent-team native-agents` when the user explicitly wants Claude CLI native subagents: the runtime creates temporary `cfc_*` subagent definitions and prompts Claude to dispatch them with `@cfc_<role>` calls. Existing `--native-agents` remains a compatibility alias.

Run:

```bash
node "${CLAUDE_PLUGIN_ROOT}/scripts/claude-companion.mjs" multi-review
```
...
```bash
node "${CLAUDE_PLUGIN_ROOT}/scripts/claude-companion.mjs" reserve-job multi-review
```
...
- `--agent-team native-agents` uses Claude CLI native subagents for the requested roles instead of separate plugin-managed role invocations.
- `--native-structured` is valid only with `--agent-team native-agents` and returns validated aggregate JSON.
- `--stream-progress` writes sanitized lifecycle progress to stderr without raw Claude response text.
...
- `--model <model>` is passed to each Claude CLI invocation.
