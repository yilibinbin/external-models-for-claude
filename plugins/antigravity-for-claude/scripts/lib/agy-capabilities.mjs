...
function assertSafeModelText(value) {
  ...
  if (/\b(gpt|openai)\b/i.test(model)) {
    throw new Error(`Antigravity for Claude does not support GPT/OpenAI models; rejected model "${model}".`);
  }
  return model;
}
...
export function selectAgyModel({ provider = "gemini", explicitModel = "", env = {}, models = null } = {}) {
  ...
  const generic = env.ANTIGRAVITY_FOR_CLAUDE_MODEL || "";
  ...
  const providerSpecific = normalizedProvider === "claude"
    ? env.ANTIGRAVITY_FOR_CLAUDE_CLAUDE_MODEL
    : env.ANTIGRAVITY_FOR_CLAUDE_GEMINI_MODEL;
  if (providerSpecific) {
    return {
      modelProvider: normalizedProvider,
