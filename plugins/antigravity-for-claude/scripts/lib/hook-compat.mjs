...
const UNSUPPORTED_HOOKS = {
  PreToolUse: "Not used because Antigravity for Claude does not intercept or rewrite tool calls; reviews run against git state instead.",
  PostToolUse: "Not used because post-tool review would add per-tool latency while duplicating the stop-time git review.",
  PermissionRequest: "Not used because this plugin must not grant, deny, or proxy Claude Code permission decisions.",
