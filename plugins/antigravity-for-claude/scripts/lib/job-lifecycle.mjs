...
export function gitSignalTimeoutMs(env = process.env) {
  return parsePositiveInteger(env.ANTIGRAVITY_FOR_CLAUDE_GIT_SIGNAL_TIMEOUT_MS, 10_000, {
    min: 100,
    max: 60_000
  });
}
...
export function jobHeartbeatIntervalMs(env = process.env) {
  return parsePositiveInteger(env.ANTIGRAVITY_FOR_CLAUDE_JOB_HEARTBEAT_INTERVAL_MS, JOB_HEARTBEAT_INTERVAL_MS, {
    min: 100,
    max: 5 * 60 * 1000
  });
}
...
export function maxActiveJobs(env = process.env) {
  return parsePositiveInteger(env.ANTIGRAVITY_FOR_CLAUDE_MAX_ACTIVE_JOBS, DEFAULT_MAX_ACTIVE_JOBS, {
    min: 1,
    max: 32
  });
}
