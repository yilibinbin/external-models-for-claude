...
const WORKFLOW_RELATIVE_PATH = path.join(".github", "workflows", "antigravity-for-claude-review.yml");
...
function readTemplate(pluginRoot) {
  return fs.readFileSync(path.join(pluginRoot, "templates", "github-actions", "antigravity-for-claude-review.yml"), "utf8");
}
...
function validateReleaseRef(value) {
  ...
  if (!/^antigravity-for-claude-v\d+\.\d+\.\d+(?:[-+][A-Za-z0-9._-]+)?$/.test(ref) && !/^[a-fA-F0-9]{40}$/.test(ref)) {
...
export function validateWorkflow(text) {
  ...
  const checks = [
    ...
    result(activeBody.includes("ANTIGRAVITY_FOR_CLAUDE_RELEASE_REF:"), "release-ref-recorded"),
    result(activeBody.includes("npm install -g @anthropic-ai/claude-code"), "claude-cli-install"),
    result(activeBody.includes("https://github.com/yilibinbin/external-models-for-claude") && activeBody.includes("claude plugin marketplace add \"$marketplace_dir\" --scope user"), "marketplace-install"),
    result(activeBody.includes("claude plugin install antigravity-for-claude@external-models-for-claude --scope user"), "plugin-install"),
    result(activeBody.includes("git -C \"$marketplace_dir\" fetch --depth 1 origin \"$ANTIGRAVITY_FOR_CLAUDE_RELEASE_REF\""), "immutable-release-ref"),
    result(activeBody.includes("claude plugin list --json") && activeBody.includes("installPath") && activeBody.includes("antigravity-for-claude@external-models-for-claude") && activeBody.includes("$CLAUDE_PLUGIN_ROOT/scripts/antigravity-companion.mjs"), "plugin-root-resolved"),
    result(activeBody.includes("ANTIGRAVITY_FOR_CLAUDE_MODEL_PROVIDER: \"{{MODEL_PROVIDER}}\"") || activeBody.includes("ANTIGRAVITY_FOR_CLAUDE_MODEL_PROVIDER:"), "provider-env"),
    result(activeBody.includes("args=(review") && activeBody.includes("$CLAUDE_PLUGIN_ROOT/scripts/antigravity-companion.mjs"), "review-command"),
    result(!activeBody.includes("node plugins/antigravity-for-claude/scripts/"), "no-repo-relative-runtime-path"),
