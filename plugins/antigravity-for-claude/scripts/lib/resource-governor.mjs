...
const PLUGIN_NAME = "antigravity-for-claude";
const DISABLE_ENV = "ANTIGRAVITY_FOR_CLAUDE_RESOURCE_GOVERNOR";
const LOCK_DIR_ENV = "ANTIGRAVITY_FOR_CLAUDE_RESOURCE_LOCK_DIR";
const MODEL_LIMIT_ENV = "ANTIGRAVITY_FOR_CLAUDE_GLOBAL_MAX_MODEL_CALLS";
const BACKGROUND_LIMIT_ENV = "ANTIGRAVITY_FOR_CLAUDE_GLOBAL_MAX_BACKGROUND_JOBS";
const MULTI_PARALLEL_ENV = "ANTIGRAVITY_FOR_CLAUDE_MULTI_REVIEW_MAX_PARALLEL";
const LOCK_WAIT_ENV = "ANTIGRAVITY_FOR_CLAUDE_RESOURCE_LOCK_WAIT_MS";
const LOCK_STALE_ENV = "ANTIGRAVITY_FOR_CLAUDE_RESOURCE_LOCK_STALE_MS";
...
export function resourceLockRoot(env = process.env) {
  ...
  return path.join(os.homedir(), ".claude", PLUGIN_NAME, "global-resource-locks");
}
...
export function resourceLimit(kind, env = process.env) {
  if (kind === "background-job") {
    return parsePositiveInteger(env[BACKGROUND_LIMIT_ENV], DEFAULT_BACKGROUND_LIMIT, { min: 0, max: 32 });
  }
  return parsePositiveInteger(env[MODEL_LIMIT_ENV], DEFAULT_MODEL_LIMIT, { min: 0, max: 32 });
}
...
export function capacityBlockedMessage(plugin, lease) {
  return `capacity_blocked: ${plugin} ${lease.kind} capacity is full (${lease.active}/${lease.limit}); wait for existing work to finish or lower concurrency.`;
}
