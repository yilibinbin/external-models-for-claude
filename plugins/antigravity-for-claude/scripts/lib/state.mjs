...
const DATA_HOME_ENV = "ANTIGRAVITY_FOR_CLAUDE_DATA";
const STATE_HOME_ENV = "ANTIGRAVITY_FOR_CLAUDE_STATE_HOME";
...
export function stateRoot(env = process.env) {
  if (env[CLAUDE_PLUGIN_DATA_ENV]) {
    return path.join(env[CLAUDE_PLUGIN_DATA_ENV], "antigravity-for-claude", "state");
  }
  if (env[DATA_HOME_ENV]) {
    return path.join(env[DATA_HOME_ENV], "state");
  }
  if (env[STATE_HOME_ENV]) {
    return env[STATE_HOME_ENV];
  }
  return path.join(os.homedir(), ".claude", "antigravity-for-claude", "state");
}
