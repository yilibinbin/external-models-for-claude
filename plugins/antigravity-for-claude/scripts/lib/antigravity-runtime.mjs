...
export const MODEL_PROVIDER_ENV = "ANTIGRAVITY_FOR_CLAUDE_MODEL_PROVIDER";
export const MODEL_ENV = "ANTIGRAVITY_FOR_CLAUDE_MODEL";
export const GEMINI_MODEL_ENV = "ANTIGRAVITY_FOR_CLAUDE_GEMINI_MODEL";
export const CLAUDE_MODEL_ENV = "ANTIGRAVITY_FOR_CLAUDE_CLAUDE_MODEL";
...
export function agyCommand(env = process.env) {
  ...
  if (env.ANTIGRAVITY_FOR_CLAUDE_TEST_DISABLE_CANDIDATE_DISCOVERY === "1") return "agy";
...
function modelProviderFromOptions(env = process.env, options = {}) {
...
export function selectedModel(env = process.env, options = {}) {
  const selected = selectAgyModel({
    provider: modelProviderFromOptions(env, options),
...
function createAgyLogFile() {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "antigravity-for-claude-"));
...
export function antigravityPrintArgs(prompt, options = {}, env = process.env) {
  ...
  const shouldCaptureLog = preflight.capabilities.logFile && env.ANTIGRAVITY_FOR_CLAUDE_DISABLE_LOG_CAPTURE !== "1";
  const args = [
    "--model",
    preflight.model,
    "--print-timeout",
  ...
  if (env.ANTIGRAVITY_FOR_CLAUDE_SANDBOX === "on" && preflight.capabilities.sandbox) {
    args.push("--sandbox");
  }
