...
description: Ask Antigravity CLI from Claude Code for read-only diagnosis when an implementation or validation run is stuck.
---

# Antigravity Rescue

Use this skill when Claude Code is blocked by failing tests, confusing runtime output, or an implementation dead end.

Run:

```bash
node "${CLAUDE_PLUGIN_ROOT}/scripts/antigravity-companion.mjs" rescue
```

## Natural-Language Model Routing

Claude Code should let the user ask for Antigravity rescue in normal language. Do not ask the user to write `--model-provider` or `--model` unless troubleshooting the plugin itself.
...
- Direct argv path: `node "${CLAUDE_PLUGIN_ROOT}/scripts/antigravity-companion.mjs" rescue --model-provider claude`.
...
- Treat the response as diagnosis and possible next steps; Claude Code decides what to change.
...
- Classify the request as rescue when Claude Code is blocked by failing tests, confusing runtime output, repeated validation failures, or an implementation dead end.
