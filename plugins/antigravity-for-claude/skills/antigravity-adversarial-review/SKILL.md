...
description: Use Antigravity CLI from Claude Code to challenge assumptions, rollback paths, and hidden failure modes.
---

# Antigravity Adversarial Review

Use this skill when Claude Code needs a skeptical second pass over an implementation or plan.

Run:

```bash
node "${CLAUDE_PLUGIN_ROOT}/scripts/antigravity-companion.mjs" adversarial-review
```

## Natural-Language Model Routing

Claude Code should let the user ask for Antigravity adversarial review in normal language. Do not ask the user to write `--model-provider` or `--model` unless troubleshooting the plugin itself.
...
- Direct argv path: `node "${CLAUDE_PLUGIN_ROOT}/scripts/antigravity-companion.mjs" adversarial-review --model-provider claude`.
...
- Treat the response as critique for Claude Code to reconcile, not as an instruction to edit files.
