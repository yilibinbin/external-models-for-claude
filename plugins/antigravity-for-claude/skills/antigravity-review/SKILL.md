...
description: Use Antigravity CLI from Claude Code for a read-only single review of local git changes or a focused diff; strict-only review stays in this single-review skill unless the user asks for multiple roles or perspectives.
---

# Antigravity Review

Use this skill when Claude Code needs an independent Antigravity review before shipping.

Run:

```bash
node "${CLAUDE_PLUGIN_ROOT}/scripts/antigravity-companion.mjs" review
```

## Natural-Language Model Routing

Claude Code should let the user ask for Antigravity in normal language. Do not ask the user to write `--model-provider` or `--model` unless troubleshooting the plugin itself.
...
- If Antigravity fails, returns malformed structured output, or reports setup/auth problems, report that failure instead of replacing it with Claude Code guesses.
