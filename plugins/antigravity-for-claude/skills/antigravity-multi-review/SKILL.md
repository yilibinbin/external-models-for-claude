...
description: Run plugin-managed Antigravity role fan-out review from Claude Code when the user asks for multiple perspectives, named roles, multi-agent or multi-role review, or high-risk multi-role review; do not use this skill for strict-only review.
---

# Antigravity Multi Review

Use this skill when Claude Code needs several independent Antigravity review passes with different role directives.

Run:

```bash
node "${CLAUDE_PLUGIN_ROOT}/scripts/antigravity-companion.mjs" multi-review
```

## Natural-Language Model Routing

Claude Code should let the user ask for Antigravity multi-role review in normal language. Do not ask the user to write `--model-provider` or `--model` unless troubleshooting the plugin itself.
...
- Direct argv path: `node "${CLAUDE_PLUGIN_ROOT}/scripts/antigravity-companion.mjs" multi-review --model-provider claude`.
- Requested roles path: `node "${CLAUDE_PLUGIN_ROOT}/scripts/antigravity-companion.mjs" multi-review --roles correctness,security`.
- Managed background path: `node "${CLAUDE_PLUGIN_ROOT}/scripts/antigravity-companion.mjs" multi-review --background`; return the job id and use the plugin job/result workflow instead of shell backgrounding.
...
- Treat each role output as review findings for Claude Code to reconcile.
- Claude Code remains responsible for deciding which findings to adopt, reject, or report as residual risk.
...
- For large diffs, slow providers, or more than three roles, prefer the plugin-managed background job path and return the job id instead of blocking the main Claude Code turn.
