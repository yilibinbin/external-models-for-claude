...
description: List and use Antigravity for Claude role packs for focused multi-review teams.
---
...
```bash
node "${CLAUDE_PLUGIN_ROOT}/scripts/antigravity-companion.mjs" roles --json
```

## Natural-Language Model Routing

Claude Code should let the user ask for Antigravity role-pack review in normal language. Do not ask the user to write `--model-provider` or `--model` unless troubleshooting the plugin itself.
...
- List packs: `node "${CLAUDE_PLUGIN_ROOT}/scripts/antigravity-companion.mjs" roles --json`.
- Direct argv path: `node "${CLAUDE_PLUGIN_ROOT}/scripts/antigravity-companion.mjs" multi-review --model-provider claude --role-pack release`.
- Role syntax: `multi-review --role-pack release` or `multi-review --roles correctness,security`.
...
- Keep the review read-only and let Claude Code reconcile findings before edits.
