...
const WORKFLOW_RELATIVE_PATH = path.join(".github", "workflows", "gemini-for-claude-review.yml");
const DEFAULT_RELEASE_REF = "gemini-for-claude-v0.1.0";
const DEFAULT_TIMEOUT_MINUTES = 30;
const LOCAL_PATH_PATTERN = /\/Users\/[A-Za-z0-9._/-]+|\/private\/var\/folders\/[A-Za-z0-9._/-]+|[A-Za-z]:\\Users\\[A-Za-z0-9._\\/-]+/;
const MARKER = "<!-- gemini-for-claude-review -->";
...
function readTemplate(pluginRoot) {
  return fs.readFileSync(path.join(pluginRoot, "templates", "github-actions", "gemini-for-claude-review.yml"), "utf8");
}
...
export function renderWorkflow(pluginRoot, options = {}) {
  ...
  const annotationStep = annotations ? [
    ...
    "        run: |",
    "          set -euo pipefail",
    "          node \"$CLAUDE_PLUGIN_ROOT/scripts/gemini-companion.mjs\" github-actions render-annotations --input gemini-for-claude-review.json > gemini-for-claude-annotations.json",
    "",
    ...
    "          script: |",
    "            const fs = require('fs');",
    "            const annotations = JSON.parse(fs.readFileSync('gemini-for-claude-annotations.json', 'utf8'));",
...
export function validateWorkflow(text, options = {}) {
  ...
  const checks = [
    ...
    result(body.includes("claude plugin install gemini-for-claude@external-models-for-claude --scope user"), "plugin-install"),
    result(body.includes("GEMINI_FOR_CLAUDE_RELEASE_REF:") && body.includes("git -C \"$marketplace_dir\" fetch --depth 1 origin \"$GEMINI_FOR_CLAUDE_RELEASE_REF\""), "immutable-release-ref"),
    result(body.includes("claude plugin list --json") && body.includes("installPath") && body.includes("gemini-for-claude@external-models-for-claude") && body.includes("$CLAUDE_PLUGIN_ROOT/scripts/gemini-companion.mjs"), "plugin-root-resolved"),
    result(!body.includes("node plugins/gemini-for-claude/scripts/"), "no-repo-relative-runtime-path"),
...
export function reviewToAnnotations(review) {
  ...
  for (const finding of findings) {
    ...
    annotations.push({
      ...
      title: sanitizeText(finding.title ?? "Gemini for Claude finding"),
