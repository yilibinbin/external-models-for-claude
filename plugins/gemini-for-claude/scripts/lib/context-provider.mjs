...
export function contextConfigPaths(env = process.env) {
  const paths = [];
  if (env.GEMINI_FOR_CLAUDE_CONTEXT_CONFIG) {
    paths.push(path.resolve(env.GEMINI_FOR_CLAUDE_CONTEXT_CONFIG));
  }
  if (env.CLAUDE_PLUGIN_DATA) {
    paths.push(path.join(path.resolve(env.CLAUDE_PLUGIN_DATA), "context", "providers.json"));
  }
  if (env.GEMINI_FOR_CLAUDE_DATA) {
    paths.push(path.join(path.resolve(env.GEMINI_FOR_CLAUDE_DATA), "context", "providers.json"));
  }
  paths.push(path.join(os.homedir(), ".claude", "gemini-for-claude", "context", "providers.json"));
  return paths;
}
