---
name: gemini-rescue
description: Ask Gemini CLI for read-only rescue diagnosis when Claude Code is stuck, failing tests, or uncertain about recovery.
---

# Gemini Rescue

Use this skill when Claude Code needs an independent Gemini diagnosis for a stuck implementation, failing validation, confusing git state, or unclear next recovery step.

Run:

```bash
node "${CLAUDE_PLUGIN_ROOT}/scripts/gemini-companion.mjs" rescue
```
...
```bash
node "${CLAUDE_PLUGIN_ROOT}/scripts/gemini-companion.mjs" reserve-job rescue
```
