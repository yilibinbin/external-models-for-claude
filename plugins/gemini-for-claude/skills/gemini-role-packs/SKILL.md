...
description: Inspect Gemini for Claude reviewer role packs and run built-in Gemini review teams.
---

# Gemini Role Packs

Use this skill when Claude Code needs a named Gemini reviewer team instead of manually listing roles.

List packs:

```bash
node "${CLAUDE_PLUGIN_ROOT}/scripts/gemini-companion.mjs" roles list
```

Inspect a built-in pack:

```bash
node "${CLAUDE_PLUGIN_ROOT}/scripts/gemini-companion.mjs" roles inspect
```

Run a built-in pack:

```bash
node "${CLAUDE_PLUGIN_ROOT}/scripts/gemini-companion.mjs" multi-review --role-pack
```

Rules:
- Role packs are Gemini for Claude reviewer presets, not Gemini extensions.
