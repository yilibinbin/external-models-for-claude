...
description: Run a bounded Gemini scorecard review loop that helps Claude Code decide whether current changes need more work.
---

# Gemini Assisted Review

Use this skill after implementation when Claude Code needs a bounded Gemini quality loop over the current git changes.
...
```bash
node "${CLAUDE_PLUGIN_ROOT}/scripts/gemini-companion.mjs" assisted-review --taskset <taskset-id>
```
