...
description: Configure the opt-in Gemini Stop review gate for Gemini for Claude.
---

# Gemini Review Gate

Use this skill when the user wants Gemini to run a Stop-time gate before Claude Code finishes a turn.

Enable the gate:

```bash
node "${CLAUDE_PLUGIN_ROOT}/scripts/gemini-companion.mjs" setup --enable-review-gate --review-gate-mode multi-role
```

Disable the gate:

```bash
node "${CLAUDE_PLUGIN_ROOT}/scripts/gemini-companion.mjs" setup --disable-review-gate
```
...
```bash
node "${CLAUDE_PLUGIN_ROOT}/scripts/gemini-companion.mjs" review-gate
```
...
- Export `GEMINI_FOR_CLAUDE_REVIEW_GATE=off` in the environment that launches Claude Code hooks to bypass the gate immediately.
...
After install or upgrade, check Claude Code Settings > Hooks and trust or enable the `Gemini for claude` Stop hook if prompted.
