...
function notifyUnreadResults(cwd, sessionId) {
  ...
  process.stderr.write(`[gemini-for-claude] Unread Gemini job result: ${summary}. Run gemini-result <job-id>.\n`);
}
...
try {
  ...
  process.stderr.write(`[gemini-for-claude unread-result] ${error.message || String(error)}\n`);
}
