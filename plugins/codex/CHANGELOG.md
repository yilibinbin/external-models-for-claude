...
- Surface the app-server's own stderr when it exits nonzero. The buffer was captured and then discarded, so a child that died mid-turn reported only `exited unexpectedly (exit 1).` — the common case being a broker-backed review. The tail is now quoted, capped at 2 KB **of UTF-8 bytes** (never splitting a code point), stripped of the routine PATH warning, and passed through `sanitizeModelText` **before** truncation, since slicing first could cut a credential in half and leave a tail the redaction patterns no longer match.
- Propagate that cause across the broker. `review/start`/`turn/start` are streaming, so once the request has returned there is no pending call left to reject and the broker previously just closed its sockets. It now sends a transport-level `broker/appServerExited` notification carrying the cause before teardown, which the broker client records as this connection's `exitError` — the value `captureTurn` rethrows.
- Wait (briefly, bounded) for stderr to drain before diagnosing. Node only guarantees drainage at `close`, and a descendant holding an inherited descriptor delays it: measured, a child exited at 29 ms with an empty buffer while its crash line arrived at 358 ms. When the grace expires the stdio handles are destroyed, because an open pipe held by a descendant otherwise keeps this process alive indefinitely.
- Broaden credential redaction, which the above made load-bearing: keyword patterns required a word boundary immediately before the keyword, so no `_`-delimited environment name (`AWS_SECRET_ACCESS_KEY`, `GITHUB_TOKEN`, `DATABASE_PASSWORD`) could ever match. Added `Authorization:`/`Proxy-Authorization:` headers in both bare and quoted form, and unterminated values — truncated output routinely yields `TOKEN="value` with no closing quote — with the lookahead keyed to the captured opening delimiter so a mixed-quote value is not mistaken for a terminated one.
