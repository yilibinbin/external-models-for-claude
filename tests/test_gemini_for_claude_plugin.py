...
def test_claude_marketplace_lists_gemini_for_claude():
...
def test_gemini_for_claude_manifest_is_claude_native():
...
def test_gemini_command_files_are_argument_safe():
...
def test_gemini_skills_do_not_publish_raw_argument_placeholders():
    for path in markdown_files("skills"):
        text = read_text(path)
        assert "$ARGUMENTS" not in text, path
        assert "<parsed-argv>" not in text, path
        assert "node plugins/gemini-for-claude/scripts/" not in text, path


def test_gemini_github_actions_templates_use_installed_plugin_runtime():
    template_paths = [
        PLUGIN / "templates" / "github-actions" / "gemini-for-claude-review.yml",
        PLUGIN / "assets" / "templates" / "github-actions" / "gemini-for-claude-review.yml",
    ]
    for path in template_paths:
        ...
        assert 'git -C "$marketplace_dir" fetch --depth 1 origin "$GEMINI_FOR_CLAUDE_RELEASE_REF"' in text
        assert 'claude plugin marketplace add "$marketplace_dir" --scope user' in text
        assert "claude plugin install gemini-for-claude@external-models-for-claude --scope user" in text
        assert "claude plugin list --json" in text
        assert "installPath" in text
        assert "gemini-for-claude@external-models-for-claude" in text
        ...
        assert "node plugins/gemini-for-claude/scripts/" not in text
        assert "$GITHUB_WORKSPACE/plugins/gemini-for-claude" not in text
        assert "$CLAUDE_PLUGIN_ROOT/scripts/gemini-companion.mjs" in text


def test_gemini_for_claude_has_no_codex_host_leakage():
...
def test_gemini_hooks_use_claude_plugin_root_and_fail_open_gate():
    ...
    assert "GEMINI_FOR_CLAUDE_REVIEW_GATE" in all_text(PLUGIN / "hooks")


def test_gemini_state_uses_claude_host_env_names():
    scripts = all_text(PLUGIN / "scripts")

    assert "CLAUDE_PLUGIN_DATA" in scripts
    assert "GEMINI_FOR_CLAUDE_DATA" in scripts
    assert "GEMINI_FOR_CLAUDE_RESOURCE_LOCK_DIR" in scripts
    assert "GEMINI_FOR_CLAUDE_REVIEW_GATE" in scripts
    assert "CODEX_PLUGIN_DATA" not in scripts
    assert "GEMINI_FOR_CODEX" not in scripts


def test_gemini_status_prefers_claude_plugin_data(tmp_path):
    plugin_data = tmp_path / "plugin-data"
    result = run_node(
        ROOT,
        "plugins/gemini-for-claude/scripts/gemini-companion.mjs",
        ["status", "--json"],
...
def test_gemini_capacity_blocked_message_omits_lock_root():
    source = (
        "const r = await import('./plugins/gemini-for-claude/scripts/lib/resource-governor.mjs');"
        "const msg = r.capacityBlockedMessage('gemini-for-claude', "
...
def test_gemini_release_check_smoke():
    result = run_node(
        ROOT,
        "plugins/gemini-for-claude/scripts/gemini-companion.mjs",
